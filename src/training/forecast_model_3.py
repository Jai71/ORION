...
def main(args):
    ...
    balanced_ds = tf.data.Dataset.zip((pos_ds, neg_ds)) \
        ...
        .shuffle(buffer_size=len(X_train_full)*2, seed=42) \
        .repeat() \
    ...
    # Keep full positive and negative pools for hard-negative mining
    X_neg_full, y_neg_full = X_neg, y_neg
    ...
    # --- Hard-negative mining training loop ---
    K = int(len(X_pos_full) * 0.5)  # number of hard negatives per epoch
    for epoch in range(args.epochs):
        print(f"🔥 Epoch {epoch+1}/{args.epochs}")
        # Train one epoch on balanced & augmented data
        model.fit(
            augmented_ds,
            epochs=1,
        ...
        # Mine hardest negatives based on model confidence
        neg_probs = model.predict(X_neg_full, batch_size=args.batch_size*4)
        # Compute anomaly confidence as max prob across anomaly classes (1-3)
        hard_scores = neg_probs[:, 1:].max(axis=1)
        # Select top-K hardest negatives
        hard_idx = np.argsort(hard_scores)[-K:]
        X_hard = X_neg_full[hard_idx]
        y_hard = y_neg_full[hard_idx]

        # Recompute steps_per_epoch for new epoch
        total_samples = len(X_pos_full) + len(X_hard)
        steps_per_epoch = total_samples // args.batch_size

        # Rebuild balanced dataset for next epoch
        def make_ds(Xa, ya):
            return tf.data.Dataset.from_tensor_slices((Xa, ya)) \
                         .shuffle(len(Xa), seed=42).map(augment, num_parallel_calls=tf.data.AUTOTUNE)
        pos_ds_epoch = make_ds(X_pos_full, y_pos_full).repeat()
        neg_ds_epoch = make_ds(X_hard, y_hard).repeat()
        balanced_ds = tf.data.Dataset.zip((pos_ds_epoch, neg_ds_epoch)) \
            .flat_map(lambda p, n: tf.data.Dataset.from_tensors(p).concatenate(tf.data.Dataset.from_tensors(n))) \
            .shuffle(buffer_size=total_samples, seed=42) \
            .repeat() \
            .prefetch(tf.data.AUTOTUNE)
        augmented_ds = balanced_ds \
            .map(lambda w,l: (w, l), num_parallel_calls=tf.data.AUTOTUNE) \
            .batch(args.batch_size) \
            .repeat() \
            .prefetch(tf.data.AUTOTUNE)
