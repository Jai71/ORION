...
def jitter_windows(X, y, anomaly_codes, k=3, max_shift=5):
    """Time-shift augmentation for anomaly windows."""
    X_aug, y_aug = [], []
    for window, code in zip(X, y):
        X_aug.append(window)
        y_aug.append(code)
        if code in anomaly_codes:
            for _ in range(k):
                ...
                if shift > 0:
                    # shift forward: remove first 'shift' rows, repeat last row
                    newwin = np.vstack([window[shift:], 
                                        np.repeat(window[-1:].reshape(1, -1), shift, axis=0)])
                elif shift < 0:
                    # shift backward: repeat first row, drop last rows
                    s = -shift
                    newwin = np.vstack([np.repeat(window[:1].reshape(1, -1), s, axis=0), 
                                        window[:-s]])
                else:
                    newwin = window.copy()
                X_aug.append(newwin)
...
def generate_patient_data(patient_id, timesteps=TIMESTEPS, anomaly_type=None):
    ...
    return data, label, anomaly_used
...
def generate_dataset(num_patients=NUM_PATIENTS, timesteps=TIMESTEPS):
    ...
    for patient_id in range(num_patients):
        ...
        data, label, anomaly_used = generate_patient_data(patient_id, timesteps, anomaly_type)
        data_list.append(data)
        labels.append(label)
        anomaly_types.append(anomaly_used)

    return data_list, labels, anomaly_types


def main():
    # Generate data for all patients
    data_list, labels, anomaly_types = generate_dataset()
    print(f"Generated synthetic data for {len(data_list)} patients with {TIMESTEPS} readings each.")

    # Build windowed dataset
    X_windows, y_codes = [], []
    for series, anomaly_type in zip(data_list, anomaly_types):
        code = TYPE_TO_CODE[anomaly_type]
        for start in range(0, TIMESTEPS - WINDOW + 1):
    ...
    # Apply jitter augmentation to anomaly windows only
    anomaly_codes = [TYPE_TO_CODE['anaphylaxis'], TYPE_TO_CODE['malignant_hyperthermia']]
    X_array, y_codes = jitter_windows(X_array, y_codes, anomaly_codes, k=3, max_shift=5)
    print(f"Applied jitter augmentation, new dataset size: {X_array.shape[0]} windows")
