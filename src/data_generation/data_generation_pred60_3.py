...
def augment_features(window):
    ...
    - First-order deltas (prepend zero row): shape WINDOW x FEATURES
    - 5-second rolling means: WINDOW x FEATURES
    - 5-second rolling stds: WINDOW x FEATURES
    Returns a (WINDOW x (4*FEATURES)) array.
    """
    # Deltas
    deltas = np.vstack([np.zeros((1, window.shape[1])), np.diff(window, axis=0)])
    # Rolling means and stds over last 5 timesteps
    ...
    # Concatenate original + deltas + means + stds -> (WINDOW, FEATURES*4)
...
def generate_patient_data(patient_id, timesteps=TIMESTEPS, anomaly_type=None):
    ...
    # Hard-negative: simulate brief benign BP dip & recovery in 5% of normals
    if anomaly_type is None and random.random() < 0.05:
        t0 = random.randint(0, timesteps - 300)
        for tt in range(t0, t0 + 60):
            time_series[tt][1] *= (1 - 0.05 * np.sin((tt - t0) / 60 * np.pi))
    ...
    if anomaly_type is not None:
        ...
        # Choose anomaly onset between 10% and 90% of the sequence
...
def main():
    ...
    # Balance on-disk dataset: downsample pure negatives to 2× positives
    pos_idx = np.where(y_codes != TYPE_TO_CODE['none'])[0]
    neg_idx = np.where(y_codes == TYPE_TO_CODE['none'])[0]
    n_pos = len(pos_idx)
    chosen_neg = np.random.choice(neg_idx, size=min(len(neg_idx), n_pos * 2), replace=False)
    keep_idx = np.concatenate([pos_idx, chosen_neg])
    X_windows = X_array[keep_idx]
    y_codes   = y_codes[keep_idx]
    # Apply feature engineering to each window
    X_array = np.stack([augment_features(w) for w in X_windows])
    # Update feature dimension for scaling
    feat_dim = X_array.shape[2]  # now equals original FEATURES * 4
    print(f"Augmented features: new window shape = {X_array.shape[1:]} (WINDOW, FEATURES')")

    # Scale engineered features
    n_windows = X_array.shape[0]
    flat = X_array.reshape(-1, feat_dim)  # shape (N * WINDOW, feat_dim)
    scaler = StandardScaler().fit(flat)
    X_scaled_flat = scaler.transform(flat)
    X_scaled = X_scaled_flat.reshape(n_windows, WINDOW, feat_dim)

    # Prepare LSTM training data (all four classes)
    X_lstm = X_scaled
    y_lstm_int = y_codes
    y_lstm = to_categorical(y_lstm_int, num_classes=4)

    # Save numpy arrays ready for model training (prediction dataset)
    np.save('data/processed/X_pred_train.npy', X_lstm)
    np.save('data/processed/y_pred_train.npy', y_lstm)
    np.save('data/processed/y_pred_labels_full.npy', y_codes)
    print(f"Saved data/processed/X_pred_train.npy with shape {X_lstm.shape}")
    print(f"Saved data/processed/y_pred_train.npy with shape {y_lstm.shape}")
    print(f"Saved data/processed/y_pred_labels_full.npy with shape {y_codes.shape}")
