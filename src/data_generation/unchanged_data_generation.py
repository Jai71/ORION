...
def save_dataset_to_csv(data_list, labels, anomaly_types, filename="data/raw/synthetic_patient_vital_signs.csv"):
    """
    Saves the dataset to a CSV file.

    The CSV will have the following columns:
      - patient_id: Patient identifier
      - timestep: Time step index for the reading
      - HR: Heart Rate value
      - BP: Blood Pressure value
      - SpO₂: Oxygen Saturation value
      - RR: Respiratory Rate value
      - Temp: Body Temperature value
      - label: 0 (normal) or 1 (anomaly)
      - anomaly_type: The type of anomaly or 'none'
    """
    rows = []
    for patient_id, (data, label, anomaly_type) in enumerate(zip(data_list, labels, anomaly_types)):
        for t in range(data.shape[0]):
            hr, bp, spo2, rr, temp = data[t]
            rows.append({
                ...
                "Temp": temp,
                "label": label,
                "anomaly_type": anomaly_type if label == 1 else "none"
            })
...
def main():
    ...
    # Save the dataset to a CSV file for further training
    save_dataset_to_csv(data_list, labels, anomaly_types)
